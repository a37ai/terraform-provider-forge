...
func TestResourceSchemaExposesOnlyImplementedProtocolsAndSafeDefaults(t *testing.T) {
	...
	for _, value := range []string{"http", "postgres"} {
	...
	var removed validator.StringResponse
	protocol.Validators[0].ValidateString(context.Background(), validator.StringRequest{ConfigValue: types.StringValue("mysql")}, &removed)
	if !removed.Diagnostics.HasError() {
		t.Fatal("unimplemented request-aware protocol mysql remains authorable")
	}
